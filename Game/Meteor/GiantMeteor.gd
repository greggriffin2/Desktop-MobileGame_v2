...
var giant_meteor_effect := preload("res://Meteor/GiantMeteorEffect.tscn")
...
func take_damage(amount: int):
	...
	if health <= 0:
		...
		var effect := giant_meteor_effect.instance()
		effect.position = position
		get_parent().add_child(effect)
		var meteor_shard := meteor.instance()
